...
[`specs/mvp.md`](specs/mvp.md); read it before implementing anything.
...
- **Agentic loop = Claude Agent SDK** (`@anthropic-ai/claude-agent-sdk`) with `cwd` = vault.
  Do **not** reimplement the loop, tools, or skills — the SDK's built-in Read/Edit/Grep/Glob,
  skills, `CLAUDE.md`, and MCP are the point.
...
- **Security:** `ANTHROPIC_API_KEY` server-side only; a single-user bearer token guards all
  endpoints; HTTPS mandatory (reverse proxy with auto-TLS).
...
global `~/.claude/CLAUDE.md`) are listed in `specs/mvp.md` §4.
