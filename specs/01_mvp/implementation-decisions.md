...
| 51 | While in Conflict, unmerged files contain the user's own version ("mine"; theirs if mine was deleted) instead of git's `<<<<<<<` markers; state/paths still come from git (unmerged index + stash) (#56). | Markers leaked into the editor, search and the AI. | yes |
| 52 | Dev/test Ollama runs with `OLLAMA_CONTEXT_LENGTH=16384` and the model has `limit: {context: 16384, output: 4096}` in the opencode provider config (#57). **This was why the local model ignored `AGENTS.md` and misused tools in most earlier chat testing.** Irrelevant for Claude in prod. | Ollama silently truncated opencode's prompt to ~2k tokens. | yes |
| 53 | Left open as **enhancements** (beyond the MVP spec, for you to decide): #59 render `![[image]]` embeds / preview images+PDFs, #60 new note from a template, #61 search: AND of words + accent-insensitive. | Scope. | — |
